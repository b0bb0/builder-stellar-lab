...
export class ScanManager {
  ...
  private async executeScan(
    ...
    try {
      ...
      this.logScanEvent(scanId, "info", "Scan started");
      ...
      for (const tool of options.tools) {
        ...
        this.logScanEvent(scanId, "info", `Starting ${tool.name} scan`);
      ...
      // Update final status
      scanQueries.updateStatus.run(
        "completed",
        "completed",
        "completed",
        scanId,
      );
      this.logScanEvent(scanId, "info", "Scan completed successfully");
  ...
  private handleScanError(scanId: string, error: Error): void {
    ...
    this.logScanEvent(scanId, "error", `Scan failed: ${error.message}`);
    console.error(`Scan ${scanId} failed:`, error);
  }

  private logScanEvent(scanId: string, level: string, message: string): void {
  ...
  async stopScan(scanId: string): Promise<boolean> {
    ...
    if (nucleiStopped) {
      ...
      this.logScanEvent(scanId, "info", "Scan stopped by user");
      this.activeScanJobs.delete(scanId);
      return true;
    }
